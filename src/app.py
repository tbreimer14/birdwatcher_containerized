...
from src.utils import generate_grad_cam, overlay_heatmap, process_canvas_mask, train_step, evaluate_model

def initialize_state():
    """Initializes and persists the core ML components across Streamlit reruns."""
    if 'train_dataset' not in st.session_state:
        st.session_state.train_dataset = TrickWaterbirdsDataset(split='train', data_dir='./data')
        st.session_state.current_idx = 0
        
    if 'test_dataset' not in st.session_state:
        st.session_state.test_dataset = TrickWaterbirdsDataset(split='test', data_dir='./data')
        ...
        if st.session_state.baseline_acc is None:
            ...
            if st.button("Evaluate Baseline"):
                with st.spinner("Calculating Baseline on Test Set..."):
                    st.session_state.baseline_acc = evaluate_model(st.session_state.model, st.session_state.test_dataset)
                st.rerun()
        else:
        ...
        if st.session_state.corrections_made >= 5:
            if st.button("Evaluate Post-Training"):
                with st.spinner("Calculating New Accuracy on Test Set..."):
                    st.session_state.new_acc = evaluate_model(st.session_state.model, st.session_state.test_dataset)
            
            if st.session_state.new_acc is not None:
                delta = st.session_state.new_acc - st.session_state.baseline_acc
                st.metric("New Test Accuracy", f"{st.session_state.new_acc:.2f}%", delta=f"{delta:.2f}%")
...
def main():
    ...
    with col2:
        ...
        current_lambda = st.slider(
            "Attention Penalty Weight (λ)", min_value=0.0, max_value=5.0, value=1.0, step=0.1
        )
        ...
        if st.button("Teach Model", use_container_width=True):
            if canvas_result.image_data is not None:
                with st.spinner("Fine-tuning model..."):
                    ...
                    user_mask = process_canvas_mask(canvas_result.image_data, target_size=(14, 14))
                    
                    loss_tot, loss_ce, loss_att = train_step(
                        model, optimizer, batch_tensor, batch_label, user_mask, lambda_weight=current_lambda
                    )
